...
const MAX_PARTICLES = 200; // Maximum total particles allowed
...
export const useBatParticles = ({
  ...
  const createParticles = useCallback(
    (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
      ...
      // Check if we need to remove old particles to stay under limit
      const currentCount = activeParticlesRef.current.length;
      if (currentCount + num > maxParticles) {
        const toRemove = currentCount + num - maxParticles;
        const oldParticles = activeParticlesRef.current.splice(0, toRemove);
      ...
      for (let i = 0; i < num; i++) {
        ...
        group.push(svg);
        activeParticlesRef.current.push(svg);
      }
      ...
      setTimeout(() => {
        group.forEach((shape) => {
          shape.remove();
          const index = activeParticlesRef.current.indexOf(shape);
          if (index > -1) {
